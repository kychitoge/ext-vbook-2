...
function execute(url) {
    ...
    if (response.ok) {
        ...
        let bookId = /readings\((\d+)\)/.exec(doc.html())[1];
        let comments = [];
        comments.push({
            title: "Đánh giá",
            input: bookId,
        ...
        return Response.error("Trang này đang lỗi hoặc đang bật cloudflare, vui lòng mở trang nguồn dạng máy tính để tiếp tục.");
    }
    return null;
}
